...
func TestReferenceImportUploadStreamsThroughTransferClient(t *testing.T) {
	...
	client.files.Transport = roundTripFunc(func(r *http.Request) (*http.Response, error) {
		...
		if r.Method != "POST" || r.URL.Path != "/base/api/reference-imports" || r.URL.Query().Get("filename") != "日本 + #&.jsonl" ||
			r.Header.Get("Authorization") != "Bearer secret" || r.Header.Get("Content-Type") != "application/x-ndjson" {
		...
		return &http.Response{StatusCode: 202, Body: io.NopCloser(strings.NewReader(`{"id":"accepted","filename":"日本 + #&.jsonl","status":"processing"}`))}, nil
	})
	result, err := client.SubmitReferenceImport(t.Context(), "日本 + #&.jsonl", body)
	...
	_, err = client.SubmitReferenceImport(t.Context(), "too-large.jsonl", strings.NewReader(""))

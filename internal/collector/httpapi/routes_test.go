...
func TestAuthenticationProtectsEveryRouteExceptHealth(t *testing.T) {
	...
	for _, path := range []string{"/api/status", "/api/inventory/status", "/api/favorites/status", "/api/metadata/status", "/api/metadata/pause", "/api/metadata/resume", "/api/metadata/lookup", "/api/metadata/fetches", "/api/metadata/fetches/job", "/api/favorites/2/sync", "/api/favorites/all/sync", "/api/downloads", "/api/downloads/1", "/api/downloads/1/file", "/api/downloads/1/retry", "/api/downloads/1/cancel", "/missing", "/healthz/"} {
...
type waitingAuthenticatedClient struct{}

func (waitingAuthenticatedClient) GetFavoritesPage(ctx context.Context, category int, next string) (panda.FavoritesPage, error) {
	<-ctx.Done()

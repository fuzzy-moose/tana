...
export function replaceListingPage(search: string, page: number) {
  window.history.replaceState(window.history.state, '', listingLink(search, page))
  window.dispatchEvent(new HashChangeEvent('hashchange'))
}

export function useGalleryLayout(search: string, page: number) {
  ...
  useLayoutEffect(() => {
    ...
    function measure() {
      ...
      if (oldSize > 0 && oldSize !== pageSize) {
        ...
        if (nextPage !== page) replaceListingPage(search, nextPage)
      }
    }
    measure()

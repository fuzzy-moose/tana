...
export function useDownloads(available: boolean, refreshKey: number) {
  const [snapshot, setSnapshot] = useState<{ jobs: DownloadJob[], offset: number } | null>(null)
  ...
  useEffect(() => {
    ...
    async function poll() {
      setChecking(true)
      try {
        const result = await listDownloads(offset, controller.signal)
        ...
        setSnapshot({ jobs: result.jobs, offset })
        setError('')
      } catch (error) {

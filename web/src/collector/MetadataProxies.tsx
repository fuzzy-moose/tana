...
import { channelInput, deleteMetadataProxy, getMetadataProxies, proxyFailure, proxyState, saveMetadataProxy, setMetadataProxies } from './metadataProxies'
import type { MetadataProxyChannel, MetadataProxyInput, MetadataProxyStatus } from './metadataProxies'
...
export default function MetadataProxies({ available, refreshKey }: { available: boolean, refreshKey: number }) {
  ...
  async function change(action: (signal: AbortSignal) => Promise<MetadataProxyStatus>, message: string) {
  ...
  return <section className="collector-panel" aria-labelledby="metadata-proxies-title">
    <div className="collector-section-heading">
      ...
      <button className="button" disabled={disabled || draft !== null} onClick={() => edit()}>Add channel</button>
    </div>
    ...
    {resource.data?.channels.length === 0 && !draft && <p className="download-empty">No proxy channels configured. Add a channel to get started.</p>}
    <div className="proxy-channels">
      {resource.data?.channels.map((channel) => <article className="proxy-channel" key={channel.id} aria-label={channel.name}>
        ...
        <div className="proxy-channel-actions">
          ...
          <div className="button-group">
            <button className="button" disabled={disabled || draft !== null || channel.state === 'removing'} onClick={() => edit(channel)}>Edit</button>
            <button className="button" disabled={disabled || draft !== null || channel.state === 'removing'} onClick={() => { void change((signal) => deleteMetadataProxy(channel.id, signal), 'Channel removal requested. Any active batch will finish first.') }}>Remove</button>
          </div>
        </div>
      </article>)}
    </div>

...
test('starts per-library and global scans; recovers active status after navigation and shows failures', async () => {
  ...
  const fetchMock = vi.fn<typeof fetch>(async (input, init) => {
    if (String(input) === '/api/libraries') return Response.json([{ id: 'l', name: 'Manga', path: '/manga', availability: 'available', last_checked_at: null }])
    if (init?.method === 'POST') {
  ...
  expect(fetchMock).toHaveBeenCalledWith('/api/scans', expect.objectContaining({ method: 'POST', body: '{"library_id":"l"}' }))
...
test('recovers the running scan after another client wins a start race', async () => {
  ...
  vi.stubGlobal('fetch', vi.fn<typeof fetch>(async (input, init) => {
    ...
    return Response.json({ phase: active ? 'discovering' : 'idle', library_id: '', discovered: 0, imported: 0, galleries_created: 0, failed_sources: 0, discovery_errors: 0, skipped: 0 })
  }))
  render(<Libraries />)
  const scan = await screen.findByRole('button', { name: 'Scan' })
  await waitFor(() => expect((scan as HTMLButtonElement).disabled).toBe(false))
